...
class MySqlQuery():
    ...
    def get_registro(self, id):
        if (id == '0'):
            ...
            query = 'select * from {0}.{1} where idempresa = {2}'.format(banco, tb_banco, int(id))
    ...
    def proximo_codigo(self):
        query = 'select max(idempresa) from {0}.{1}'.format(banco, tb_banco)
